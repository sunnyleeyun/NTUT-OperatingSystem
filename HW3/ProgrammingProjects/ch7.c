...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < NUMBER_OF_RESOURCES; ++i)
    {
        ...
        {
            maximum[j][i] = MAX_RESOURCE;    /* random initializations for maximum allowed requests*/
            need[j][i] = MAX_RESOURCE;       /*The need should equal the maximum*/
...
void* getResources(void *arg)
{
    ...
    // for (i = 0; i < NUMBER_OF_RESOURCES; ++i) {
    //     for (j = 0; j < NUMBER_OF_CUSTOMERS; ++j) {
    //         request[i][j] = need[i][j] <= 0 ? need[i][j] : rand() % (need[i][j] + 1);
    //     }
    // }

    // for (i = 0; i < NUMBER_OF_RESOURCES; ++i) {
    //     for (j = 0; j < NUMBER_OF_CUSTOMERS; ++j) {
    //         while(request_resources_highlevel(&request[i][j],customer_num)==false);
    //     }
    // }
